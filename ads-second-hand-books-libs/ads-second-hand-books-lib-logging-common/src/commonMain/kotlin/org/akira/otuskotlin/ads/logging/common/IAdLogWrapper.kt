...
interface IAdLogWrapper : AutoCloseable {
    val loggerId: String
    ...
    /**
     * Обертка для декорирования кода с логированием перед выполнением и после
     */
    ...
    suspend fun <T> doWithLogging(
        ...
        log(
            ...
            level = level,
            objs = mapOf("metricHandlerTime" to diffTime.toIsoString())
        )
        res
    } catch (e: Throwable) {
        log(
    ...
    /**
     * Обертка для декорирования кода с логированием ошибки
     */
    ...
    companion object {
        val DEFAULT = object : IAdLogWrapper {
