...
fun validationAuthorsCorrect(command: AdCommand, processor: AdProcessor) = runTest {
    val ctx = AdContext(
        command = command,
        state = AdState.NONE,
    ...
    assertEquals("test", ctx.adValidated.authors)
}

fun validationAuthorsTrim(command: AdCommand, processor: AdProcessor) = runTest {
    val ctx = AdContext(
        command = command,
        state = AdState.NONE,
        workMode = AdWorkMode.TEST,
...
fun validationAuthorsEmpty(command: AdCommand, processor: AdProcessor) = runTest {
    val ctx = AdContext(
        command = command,
        state = AdState.NONE,
        workMode = AdWorkMode.TEST,
    ...
    assertEquals("authors", error?.field)
    assertContains(error?.message ?: "", "authors")
}

fun validationAuthorsSymbols(command: AdCommand, processor: AdProcessor) = runTest {
    val ctx = AdContext(
        command = command,
        state = AdState.NONE,
        workMode = AdWorkMode.TEST,
