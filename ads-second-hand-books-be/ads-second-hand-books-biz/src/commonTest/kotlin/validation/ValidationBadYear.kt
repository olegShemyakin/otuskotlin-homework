...
fun validationYearCorrect(command: AdCommand, processor: AdProcessor) = runTest {
    val ctx = AdContext(
        command = command,
        state = AdState.NONE,
...
fun validationYearNegative(command: AdCommand, processor: AdProcessor) = runTest {
    val ctx = AdContext(
        command = command,
        state = AdState.NONE,
        workMode = AdWorkMode.TEST,
    ...
    assertContains(error?.message ?: "", "year")
}

fun validationYearFuture(command: AdCommand, processor: AdProcessor) = runTest {
    val yearFuture = Clock.System.now().toLocalDateTime(TimeZone.currentSystemDefault()).year + 1
    val ctx = AdContext(
        command = command,
        state = AdState.NONE,
        workMode = AdWorkMode.TEST,
