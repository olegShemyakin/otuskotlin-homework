...
fun ICorChainDsl<AdContext>.stubUpdateSuccess(title: String, corSettings: AdCorSettings) = worker {
    this.title = title
    this.description = "Успешное изменение объявления"
    ...
    handle {
        logger.doWithLogging(id = this.requestId.asString(), LogLevel.DEBUG) {
            ...
            val stub = AdStub.prepareResult {
                adRequest.id.takeIf { it != AdId.NONE }?.also { this.id = it }
