...
class Exercise1Filter {
    ...
    fun filter() = runBlocking {
        ...
        val res = LIST
            .asFlow()
            .filter {
                flt.title?.let { t -> t == it.title } ?: true
            }
            .filter {
                flt.type?.let { t -> t == it.type} ?: true
            }
            .filter {
                flt.priceMin?.let { price -> it.price >= price } ?: true
            }
            .filter {
                flt.priceMax?.let { price -> it.price <= price } ?: true
            }
            .filter {
                flt.visibilitiesOr?.contains(it.visibility) ?: true
            }
